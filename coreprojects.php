...
<body>
  ...
  $project_select = "SELECT * FROM  tblcoreprojects c JOIN tblcorereas a ON c.areaID = a.areaID ";
  // echo "".$project_select.""; exit;
  if (!empty($areaID)) {
    $project_select .= " WHERE c.areaID = $areaID ";
  }
  ...
  if (!empty($areaID)) {
    ...
    $project_quer = mysqli_query($con, $project_select);
    while ($row = mysqli_fetch_array($project_quer)) { ?>
      <section id="main-container" class="main-container">
        <div class="container">
          <div class="row">
            <a href="?aid=<?php echo $row['areaID'] ?>">
              <div class="col-lg-4 col-md-4 col-sm-6 mb-5">
                <div class="item">
                  <div class="ts-team-wrapper">
                    <div class="team-img-wrapper ">
                      <img loading="lazy" src="app/project_previews/<?php echo $row['projectPhoto']; ?>"
                        class="img-guard img-fluid" alt="team-img">
                    </div>
                    <div class="ts-team-content-classic">
                      <h2 class="service-box-title"><a href="?aid=<?php echo $row['areaID']; ?>"><?php echo $row['projectTitle']; ?></a></h2>
                      <p style="margin-top: -5px">
                        <?php echo substr($row['projectDesc'], 0, 177) ?><b>...</b>
                      </p>
                    </div>
                  </div>
                </div>
